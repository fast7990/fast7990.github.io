class AudioRecorder {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
      return navigator.mediaDevices
        ...
        .catch((error) => {
          ...
          layer.open({
            skin: "msg",
            content: `需要麦克风权限才能进行录音，请在设置中允许应用访问麦克风 ${error.name}: ${error.message}`,
            duration: 2000,
          });
          throw error;
        });
        ...
        try {
          ...
          if (data.status === "realtime") {
            ...
            if (index !== -1) {
              ...
              console.log("实时识别片段2:", this.recognitionResultList[index].content);
            } else {
              // 未找到，推入新项
  ...
  // 获取实时结果
  getRealTimeResult() {
    return Array.isArray(this.recognitionResultList) ? [...this.recognitionResultList] : [];
  }
