...
import commonPinia from "@/packages/pinia/common.ts"
...
function hasUserInfo() {
    const appStore = appPinia()
    // const cookie = Cookies.get(configSet.SSO_COOKIE_NAME);
...
function getUserInfo(to: RouteLocationNormalized, from: RouteLocationNormalized, next: NavigationGuardNext) {
    ...
    commonStore.initUserInfo().then(res => {
        const commonStore = commonPinia()
        if (res) {
            updateRouterAll(to, from, next)
        }
        ...
        // next(appStore.configOptions.resetPath)
        commonStore.redirectToLogin() //跳登录
    })
}
